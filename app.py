...
st.set_page_config(
    page_title="Simulateur SASU/EURL",
    page_icon="💼",
    layout="wide"
)
...
def afficher_resultat(col, resultat: ResultatSimulation, couleur: str):
    with col:
        ...
        st.metric(
            "📉 Taux de prélèvement global",
            f"{resultat.taux_prelevement_global*100:.1f}%"
        )
        ...
        with st.expander("Voir le détail du calcul"):
            ...
            st.write(f"- Charges exploitation : {resultat.charges_exploitation:,.0f} €".replace(",", " "))
            st.write(f"- Bénéfice avant rémunération : {resultat.benefice_avant_remuneration:,.0f} €".replace(",", " "))
            
            st.markdown("**Rémunération**")
            st.write(f"- Rémunération nette : {resultat.remuneration_nette:,.0f} €".replace(",", " "))
            st.write(f"- Charges sociales : {resultat.charges_sociales:,.0f} €".replace(",", " "))
            
            st.markdown("**Résultat société**")
            st.write(f"- Bénéfice après rémunération : {resultat.benefice_apres_remuneration:,.0f} €".replace(",", " "))
            ...
            if resultat.dividendes_bruts > 0:
                ...
                st.write(f"- Dividendes bruts : {resultat.dividendes_bruts:,.0f} €".replace(",", " "))
                if resultat.charges_dividendes > 0:
                    st.write(f"- Charges sociales dividendes : {resultat.charges_dividendes:,.0f} €".replace(",", " "))
                st.write(f"- Impôt dividendes : {resultat.impot_dividendes:,.0f} €".replace(",", " "))
                st.write(f"- Dividendes nets : {resultat.dividendes_nets:,.0f} €".replace(",", " "))
            
            st.markdown("**Impôt sur le revenu**")
            st.write(f"- Revenu imposable : {resultat.revenu_imposable_ir:,.0f} €".replace(",", " "))
...
fig.add_trace(go.Bar(
    ...
    x=categories,
    y=[resultat_sasu.charges_sociales + resultat_sasu.charges_dividendes,
...
fig.add_trace(go.Bar(
    ...
    x=categories,
    y=[resultat_sasu.ir_total + resultat_sasu.impot_dividendes,
...
st.info(f"💡 Avec ces paramètres, **{meilleur}** vous permet de gagner **{abs(diff):,.0f} €** de plus par an ({abs(diff)/12:,.0f} €/mois)".replace(",", " "))
